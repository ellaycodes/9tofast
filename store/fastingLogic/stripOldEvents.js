...
export function removeFutureEvents(events = [], currentTime = new Date()) {
  ...
  if (futureEvents.length > 0) {
    logWarn(
      "[fasting-persistence] Ignoring events too far in the future:",
      futureEvents
    );
  }
  return validEvents;
...
export function removeOldEventsAndHandleMidnight(
  events = [],
  ...
  const eventsBeforeToday = recentEvents.filter(
    (event) => event.ts < startOfToday
  );
  let eventsFromToday = recentEvents.filter(
    (event) => event.ts >= startOfToday
  );
  ...
  const fastingContinuedOvernight =
    eventsBeforeToday.length > 0 && isFasting(eventsBeforeToday);
  const hasMidnightStart = eventsFromToday.some(
    (event) => event.type === EVENT.START && event.ts === startOfToday
  );

  if (fastingContinuedOvernight && !hasMidnightStart) {
